...
class perturbed(object):
	...
	def save_img(self, m, n, fold_curve='fold', repeat_time=4, relativeShift_position='relativeShift_v2'):
		...
		for repeat_i in range(repeat_time):
			...
			''''''
			perturbed_v = np.array([random.randint(-12000, 12000) / 100, random.randint(-12000, 12000) / 100])
			''''''
			if fold_curve == 'fold':
		...
		if is_save_perturbed:
			...
			if is_save_perturbed:
				...
				# 	if self.is_perform(0, 1):
				# if self.is_perform(0.8, 0.2):
		...
		for n in range(args.sys_num, args.sys_num_e):
			...
			for m_n in range(10):
				try:
					...
					repeat_time = min(max(round(np.random.normal(10, 4)), 1), 16) 
					process_pool.apply_async(func=saveFold.save_img, args=(m, n, 'fold', repeat_time, 'relativeShift_v2'))

					repeat_time = min(max(round(np.random.normal(8, 4)), 1), 16)
...
if __name__ == '__main__':
	...
	parser.add_argument('--sys_num_e', default=14, type=int)
